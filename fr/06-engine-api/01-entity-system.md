...
```c
bool ConfigGetBool(string entryName);   // methode de script normale : renvoie ConfigGetInt(entryName) == 1
...
```c
ComponentEnergyManager GetCompEM();   // methode de script normale (renvoie m_EM ou caste le composant gestionnaire d'energie), pas proto native
```
