...
`Timer` no tiene un método `IsPaused()`. Como `IsRunning()` devuelve `true` solo mientras el temporizador está activo (y `false` una vez pausado o detenido), úsalo para decidir si pausar o continuar:
