...
**Chase Group** (`DayZInfectedAttackGroupTipus.CHASE`): Running attacks at 2.4m range, no cooldown reduction, always pitch -1. Two variants: left and right.
...
| **Network** | Four synced variables per zombie (`m_MindState`, `m_OrientationSynced`, `m_MovementSpeed`, `m_IsCrawling`). Valtozass trigger `SetSynchDirty()`. |
