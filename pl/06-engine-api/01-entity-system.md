...
```c
bool ConfigGetBool(string entryName);   // zwykla metoda skryptowa: zwraca ConfigGetInt(entryName) == 1
...
```c
ComponentEnergyManager GetCompEM();   // zwykla metoda skryptowa (zwraca m_EM lub rzutuje komponent menedzera energii), nie proto native
```
