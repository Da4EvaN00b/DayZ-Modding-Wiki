...
**Protection:** Gas masks with intact filters and NBC suits provide protection. The damage tick is handled in script by the `ContaminatedTrigger` class (using `GameConstants.AI_CONTAMINATION_DMG_PER_SEC`), and protection comes from worn NBC equipment --- neither is set in the JSON configuration.
