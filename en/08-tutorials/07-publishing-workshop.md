...
│   └── MyMod.bikey
├── mod.cpp
└── meta.cpp  (created automatically when you publish; not something you write)
```
...
| `meta.cpp` | Workshop metadata. Bohemia documents it as created automatically during mod publishing -- it carries the Workshop ID, so you do not author or edit it by hand |
...
Those ten keys are the set Bohemia documents for `mod.cpp`. There is no published list of "required" fields -- `mod.cpp` is itself documented as optional -- so treat a missing key as a blank in the launcher rather than an error, and fill in the ones your mod's presentation actually needs.

### Field Reference

| Field | Description |
|-------|-------------|
| `name` | Display name shown in the DayZ Launcher mod list |
| `picture` | Main image, shown in the expanded mod description |
| `logo` | Logo shown below the game menu |
| `logoSmall` | Icon next to the mod name when the description is not expanded |
| `logoOver` | Logo variant used on mouse hover |
| `tooltip` | Short one-line description shown on hover |
| `overview` | Longer description shown in the mod details panel |
| `action` | URL opened from the mod entry -- typically your Workshop page or repository |
| `author` | Your name or team name |
| `version` | Free-form version string. Nothing in the game's scripts reads it, so use it for humans |

> You will see `overviewPicture` in guides and in some published mods. It is not in Bohemia's documented key set, and it appears nowhere in the extracted game data or in the public mods checked for this chapter, so do not rely on it. The documented way to get a large image into the expanded description is `picture`.
>
> The in-game main menu builds its mod list from the `CfgMods` entry in `config.cpp` rather than from `mod.cpp` -- the game's `ModStructure` reads `name`, `picture`, `logo`, `logoSmall`, `logoOver`, `tooltip` and `overview` from there. If your mod shows up unnamed in-game but fine in the launcher, that is the place to look.

### Common Mistakes

- **Missing semicolons** at the end of each line. Every line must end with `;`.
- **Wrong image paths.** The path has to be the in-game path the packed PBO exposes, not a disk path. Load the mod locally and look at the launcher entry before uploading -- a wrong path shows as a blank image, not as an error.
- **Forgetting to update the version** before re-uploading. Nothing enforces it, which is exactly why it gets missed; it is how your users tell releases apart.
...
| Image | Suggested size | Format | Used For |
|-------|----------------|--------|----------|
| Mod logo (`picture` / `logo`) | 512 x 512 px | `.paa` (in-game) | Launcher mod list and expanded description |
...
Sizes here are practical suggestions, not published requirements -- Bohemia does not document dimensions for these. Keep textures to power-of-two dimensions (256, 512, 1024) as elsewhere in the engine.
...
1. **Download the mod** using SteamCMD or the Steam client. Workshop downloads for DayZ need a real account -- `+login anonymous` does not work here, so substitute your own Steam login:
   ```batch
   steamcmd +login your_username +workshop_download_item 221100 WORKSHOP_ID +quit
   ```
   `221100` is the DayZ **client** App ID, which is the one Workshop items belong to. See [Chapter 9.10: Mod Management](../09-server-admin/10-mod-management.md) for the download-and-symlink procedure in full, and [Chapter 9.1: Server Setup](../09-server-admin/01-server-setup.md) for what is and is not established about the account requirement.
