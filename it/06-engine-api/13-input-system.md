...
La classe `Input` fornisce anche varianti `_ID` che accettano ID di azione interi invece di stringhe (es. `LocalPress_ID(int action, bool check_focus = true)`). `UAInput` non ha overload `_ID`; i suoi metodi (`LocalPress()`, ecc.) non accettano parametri.
