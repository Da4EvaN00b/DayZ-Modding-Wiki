...
```c
bool ConfigGetBool(string entryName);   // 通常のスクリプトメソッド: ConfigGetInt(entryName) == 1 を返す
...
```c
ComponentEnergyManager GetCompEM();   // 通常のスクリプトメソッド (m_EM を返すか、エネルギーマネージャーコンポーネントをキャストする)、proto native ではない
```
