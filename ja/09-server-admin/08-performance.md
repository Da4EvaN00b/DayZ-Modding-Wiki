...
サーバーゲームループは負荷に応じて変動する可変FPSで動作します。サーバーが健全なFPS（`serverFpsWarning` のデフォルトしきい値は15）を維持できない場合、プレイヤーはデシンクを経験します -- ラバーバンディング、アイテムピックアップの遅延、ヒット登録の失敗。15サーバーFPS以下では、ゲームはプレイ不能になります。
...
| `SpawnInitial` | 1200 | 初期アイテムスポーン中に許可されるスポーン試行（テスト）回数であり、スポーンされるアイテム数ではありません。初回起動時にスポーンされるルート量は `InitialSpawn`（デフォルト100、パーセンテージ）によって制御されます。値が高いほど初期ロードが長くなります。定常状態のパフォーマンスには影響しません。 |
...
**変更できないこと:** サーバーFPSの最低値を強制的に高く設定する方法はありません。サーバーFPSは可変で、負荷に応じて変動します。`-limitFPS=` 起動パラメータで最大値を制限（現在の最大値は200）して、人口の少ないサーバーでCPU使用率を下げることはできますが、負荷下でサーバーが処理に追いつけない場合は、単に遅く動作します。
