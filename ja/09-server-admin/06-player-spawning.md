...
このファイルはミッションフォルダ（例: `dayzOffline.chernarusplus/cfgplayerspawnpoints.xml`）にあります。3つのセクションがあり、それぞれ独自のパラメータと位置バブルを持ちます:

- **`<fresh>`** -- 新しいキャラクター（初回ライフまたは死亡後）
- **`<hop>`** -- サーバーホッパー（プレイヤーが別のサーバーにキャラクターを持っていた場合）
- **`<travel>`** -- ゲーム内マップ移動/テレポートスポーン
...
| パラメータ | 値 | 意味 |
|-----------|-------|---------|
| `grid_density` | 4 | グリッドのサンプリング頻度（分割数） -- 高いほど候補が多く、CPU負荷が高い。ポイント間の間隔 = `grid_width` / `grid_density` |
| `grid_width` | 200 | 候補グリッドの総幅（メートル、バブルを中心とする） -- X軸で両側に約100m展開 |
| `grid_height` | 200 | 候補グリッドの総高さ（メートル、バブルを中心とする） -- Z軸で両側に約100m展開 |
| `min_steepness` / `max_steepness` | -45 / 45 | 地形の傾斜範囲（度） -- 崖面や急な丘を除外します |

各バブルは候補ポイントが `grid_width` / `grid_density` = 200/4 = 50m 間隔で配置された200x200mのグリッド（おおよそ約16-25候補）を取得します。エンジンは傾斜と静的距離でフィルタリングし、スポーン時に `spawn_params` を適用します。
...
| `lifetime` | 120 | システムが別のグループに切り替わるまでスポーングループがアクティブのままでいる秒数。-1 = 無効 |
| `counter` | 2 | グループが切り替えられるまでアクティブのままでいるログイン回数（グループごと）。-1 = 無効 |

`lifetime` はシステムが別のグループに切り替わる前に、スポーングループがアクティブグループのままでいる時間を制御します。これは位置ごとのロックアウトではありません。同時にスポーンするプレイヤー間の間隔は `min_dist_player` によって強制されます。
...
各プレイヤーに与えられるもの: **BandageDressing**（クイックバー2）、ランダムな **Chemlight**（クイックバー1）、ランダムなフルーツ -- 35% Apple、30% Plum、35% Pear（クイックバー3）。`SetRandomHealth` はすべてのアイテムに45-65%のコンディションを設定します。
...
バランスの取れたスポーンのために、複数のプレイヤーが同時に死亡した時に単一のグループが `min_dist_player` を満たすのに十分な広がりを保てるよう、グループごとに少なくとも4つの位置を維持してください。
...
2-3つの位置しかない場合、アクティブグループがプレイヤーを分散させることができず、クラスタリングが発生します。バニラは11グループに49のfresh位置を使用しています。4つ以上のグループに少なくとも20の位置を目指してください。
...
1-2つの位置しかないグループは、エンジンが選択する位置を変化させるための候補が少なすぎます。グループごとにさらに位置を追加してください。
