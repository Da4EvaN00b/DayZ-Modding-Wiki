...
| 2304 | UDP | クライアント接続ポート（`clientPort`） |
...
types.xml内のすべての `<category>`、`<usage>`、`<value>` タグは、**cfglimitsdefinition.xml** で定義された名前と一致する必要があります。定義が `Military`（大文字M）なのに `usage name="military"`（小文字）というタイプミスがあると、アイテムは警告なしにスポーンしません。
...
- **ゾンビが多すぎる** -- **globals.xml** の `ZombieMaxCount` を減らす（デフォルト1000、400-600を試す）
