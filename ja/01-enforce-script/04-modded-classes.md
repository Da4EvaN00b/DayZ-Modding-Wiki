...
Modが別のModをオプションでサポートする場合、プリプロセッサガードを使用します。 If the other mod declares a symbol in its `config.cpp` (via the `defines[]` array in `CfgMods`), you can check for it at compile time.
