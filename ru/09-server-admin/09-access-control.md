...
Файл **ban.txt** находится в корневом каталоге вашего сервера. Он содержит по одному UID игрока на строку:
...
- Каждая строка -- это 44-символьный UID игрока DayZ -- не 17-значный SteamID64. UID игрока можно найти в логах `*.ADM` и `*.RPT`.
- Вы можете добавить комментарий после ID, используя префикс `//` на той же строке, или на отдельной закомментированной строке.
- Игрокам, чей UID указан в этом файле, отказывается подключение при попытке входа.
- Использование **ban.txt** можно переключить параметром `disableBanlist` в **serverDZ.cfg** (по умолчанию `false`).
...
Файл **whitelist.txt** находится в том же корневом каталоге сервера. При включении белого списка (`enableWhitelist = 1` в **serverDZ.cfg**) подключаться могут только игроки, чей UID указан в этом файле:
...
Формат идентичен **ban.txt** -- один 44-символьный UID игрока на строку, с необязательными комментариями `//`.
...
BattlEye включён по умолчанию. Вы запускаете сервер с помощью `DayZServer_x64.exe`, и BattlEye загружается автоматически. Для явного отключения (не рекомендуется для продакшена) установите `BattlEye = 0;` в **serverDZ.cfg**.
...
BattlEye не использует фиксированный порт RCON по умолчанию -- если вы опустите `RConPort`, он будет слушать случайный порт. Задайте его явно. Рекомендуемое значение -- ваш игровой порт плюс 3, то есть `2305` для сервера, работающего на порту `2302`. Он не должен совпадать с игровым портом или портом запросов Steam.
...
| Использование SteamID64 вместо UID игрока в **ban.txt** | Баны не работают | Используйте 44-символьный UID игрока, по одному на строку (комментарии после `//` разрешены) |
...
| Забыли открыть порт RCON в файрволе | RCON-клиент получает таймаут | Откройте UDP-порт RCON (тот, что вы задали параметром `RConPort`, например `2305`) в вашем файрволе |
| Редактирование **bans.txt** в `BattlEye/` с UID игроков | Баны не работают | BattlEye **bans.txt** использует GUID, а не UID; используйте **ban.txt** в корне сервера для банов по UID |
