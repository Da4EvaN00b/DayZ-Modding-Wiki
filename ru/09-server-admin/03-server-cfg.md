...
| `lightingConfig` | int | 0, 1, 2 | 0 | 0 = более светлые ночи (виден лунный свет). 1 = кромешно-тёмные ночи (требуется фонарик/ПНВ). 2 = освещение для Sakhal. |
...
| `serverTimeAcceleration` | float | 0.1-64 | 12 | Множитель внутриигрового времени. При 12 полный цикл 24 часа занимает 2 реальных часа. При 1 время реальное. При 24 полный день проходит за 1 час. |
