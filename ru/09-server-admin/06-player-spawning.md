...
Этот файл находится в вашей папке миссии (например, `dayzOffline.chernarusplus/cfgplayerspawnpoints.xml`). Он имеет три раздела, каждый со своими параметрами и пузырями позиций:

- **`<fresh>`** -- новые персонажи (первая жизнь или после смерти)
- **`<hop>`** -- серверхопперы (игрок имел персонажа на другом сервере)
- **`<travel>`** -- спавны при внутриигровом перемещении/телепортации по карте
...
| Параметр | Значение | Описание |
|-----------|-------|---------|
| `grid_density` | 4 | Частота выборки (число подразделений) сетки -- выше = больше кандидатов, выше нагрузка на CPU. Расстояние между точками = `grid_width` / `grid_density` |
| `grid_width` | 200 | Общая ширина сетки кандидатов в метрах (центрирована на пузыре) -- простирается на ~100 м в каждую сторону по оси X |
| `grid_height` | 200 | Общая высота сетки кандидатов в метрах (центрирована на пузыре) -- простирается на ~100 м в каждую сторону по оси Z |
| `min_steepness` / `max_steepness` | -45 / 45 | Диапазон уклона рельефа в градусах -- отклоняет обрывы и крутые холмы |

Каждый пузырь получает сетку 200x200 м с точками-кандидатами, расположенными на расстоянии `grid_width` / `grid_density` = 200/4 = 50 м друг от друга (порядка ~16-25 кандидатов). Движок фильтрует по крутизне и расстоянию до статических объектов, затем применяет `spawn_params` в момент спавна.
...
| `lifetime` | 120 | Секунды, в течение которых группа спавна остаётся активной, прежде чем система переключится на другую группу. -1 = отключено |
| `counter` | 2 | Число входов, в течение которых группа остаётся активной, прежде чем будет переключена (для каждой группы). -1 = отключено |

`lifetime` определяет, как долго группа спавна остаётся активной, прежде чем система переключится на другую группу; это не блокировка отдельной позиции. Расстояние между одновременными спавнами обеспечивается параметром `min_dist_player`.
...
Что каждый игрок получает: **BandageDressing** (быстрый слот 2), случайный **Chemlight** (быстрый слот 1), случайный фрукт -- 35% Apple, 30% Plum, 35% Pear (быстрый слот 3). `SetRandomHealth` устанавливает 45-65% состояния для всех предметов.
...
Для сбалансированного спавна держите минимум 4 позиции на группу, чтобы одна группа имела достаточный разброс для удовлетворения `min_dist_player`, когда несколько игроков погибают одновременно.
...
С только 2-3 позициями активная группа не может развести игроков, и возникает скопление. Ванильная версия использует 49 свежих позиций в 11 группах. Стремитесь к минимум 20 позициям в 4+ группах.
...
Группы с 1-2 позициями имеют слишком мало кандидатов, чтобы движок мог варьировать выбранную позицию. Добавьте больше позиций в группу.
