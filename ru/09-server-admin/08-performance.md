...
Игровой цикл сервера работает с переменной частотой FPS, которая колеблется в зависимости от нагрузки. Когда сервер не может поддерживать здоровый FPS (порог по умолчанию `serverFpsWarning` равен 15), игроки испытывают рассинхронизацию -- телепортацию, задержку подбора предметов и сбои регистрации попаданий. Ниже 15 серверных FPS игра становится неиграбельной.
...
| `SpawnInitial` | 1200 | Количество попыток спавна (тестов), разрешённых во время начального спавна предметов, а не количество заспавненных предметов. Объём лута, спавнящегося при первом запуске, управляется параметром `InitialSpawn` (по умолчанию 100, это процент). Более высокие значения означают более длительную начальную загрузку. Не влияет на стабильную производительность. |
...
**Что нельзя изменить:** нет настройки, чтобы заставить сервер держать более высокий минимальный FPS. Серверный FPS является переменным и колеблется в зависимости от нагрузки. Вы можете ограничить максимум с помощью параметра запуска `-limitFPS=` (текущий максимум 200), чтобы снизить нагрузку на ЦП на серверах с малым числом игроков, но если сервер не справляется под нагрузкой, он просто работает медленнее.
