...
| 2303 | UDP | Сеть Steam |
| 2304 | UDP | Порт подключения клиента (`clientPort`) |
...
Каждый тег `<category>`, `<usage>` и `<value>` в вашем types.xml должен совпадать с именем, определённым в **cfglimitsdefinition.xml**. Опечатка вроде `usage name="military"` (с маленькой) при определении `Military` (с большой M) молча предотвращает появление предмета.
...
- **Слишком много зомби** -- снизьте `ZombieMaxCount` в **globals.xml** (по умолчанию 1000, попробуйте 400-600)
