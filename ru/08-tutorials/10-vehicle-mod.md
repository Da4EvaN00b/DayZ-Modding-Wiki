...
**OnDebugSpawn** -- Вызывается при использовании меню отладочного спавна. `SpawnUniversalParts()` добавляет лампочки фар и автомобильный аккумулятор. `FillUpCarFluids()` заполняет топливо, охлаждающую жидкость и масло до максимума (тормозную жидкость он не заполняет). Затем мы создаём колёса, двери, капот и багажник. Это даёт вам сразу управляемый транспорт для тестирования.
