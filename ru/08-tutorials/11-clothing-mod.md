...
**Справочные значения ванильного heatIsolation:** Футболка 0.1, Худи 0.6, Куртка Горка 0.4, Туристическая куртка 0.8, Шерстяное пальто 0.9.

**Ремонт:** `repairableWithKits[] = { 5, 2 }` перечисляет типы наборов (5=Скотч, 2=Швейный набор). `repairCosts[]` указывает расход материала на ремонт в соответствующем порядке.
...
При расширении `GorkaEJacket_ColorBase` вы наследуете его сетку инвентаря (`itemsCargoSize[] = { 6, 4 }`) и слот инвентаря (`"Body"`) автоматически. Свойство `itemSize[] = { 3, 4 }` определяет размер куртки при хранении как лута -- а НЕ её грузоподъёмность.
