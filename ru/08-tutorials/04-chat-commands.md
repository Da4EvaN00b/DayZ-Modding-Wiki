...
Метод `OnEvent` класса `MissionGameplay` вызывается для различных игровых событий. Когда `eventTypeId` равен `ChatMessageEventTypeID`, это означает, что игрок только что отправил сообщение в чат. Параметры представляют собой `ChatMessageEventParams` (то есть `Param4<int, string, string, string>`) и содержат:

- `param1` -- Канал (int): канал чата (глобальный, прямой и т.д.)
- `param2` -- Имя отправителя (string)
- `param3` -- Текст сообщения (string)
- `param4` -- Класс конфигурации цвета (string)
...
|-------|------|----------------------|
| `"colorStatusChannel"` | Синий | Системные сообщения |
| `"colorAction"` | Жёлтый | Обратная связь по действиям |
...
- **GetPlainId() vs GetId():** `GetPlainId()` возвращает Steam64 ID в виде открытого текста. `GetId()` возвращает стабильный хешированный уникальный ID (безопасный для баз данных и логов), а не ID сессии -- ID для конкретной сессии, который переиспользуется после отключения игрока, это `GetPlayerId()` (int). Используйте `GetPlainId()` для проверок администратора.
...
- Хук `OnEvent` перехватывает сообщение, но не подавляет его отправку как обычного чата. Чтобы подавить его в продакшн-моде, необходимо модифицировать класс `ChatInputMenu` для фильтрации сообщений с `/` перед отправкой. В ванили `ChatInputMenu` отправляет текст чата из своего обработчика `OnChange()`, где вызывает `g_Game.ChatPlayer(text)`. Вы можете переопределить `OnChange()` и пропустить отправку, когда текст начинается с `/`:
...
modded class ChatInputMenu
{
    ...
    {
        // Получить текущий текст из виджета ввода (m_edit_box.GetText())
        // Если начинается с /, НЕ вызывать super (который вызывает g_Game.ChatPlayer)
...
- **Используйте `GetPlainId()` для проверок администратора, а не `GetId()`.** `GetId()` возвращает стабильный хешированный уникальный ID, предназначенный для баз данных и логов (ID для конкретной сессии, который переиспользуется после отключения игрока, это `GetPlayerId()`). `GetPlainId()` возвращает Steam64 ID в виде открытого текста.
