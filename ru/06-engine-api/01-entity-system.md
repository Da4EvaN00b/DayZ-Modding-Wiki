...
```c
bool ConfigGetBool(string entryName);   // обычный скриптовый метод: возвращает ConfigGetInt(entryName) == 1
...
```c
ComponentEnergyManager GetCompEM();   // обычный скриптовый метод (возвращает m_EM или приводит компонент менеджера энергии), не proto native
```
