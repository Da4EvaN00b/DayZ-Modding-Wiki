...
Когда ваш мод опционально поддерживает другой мод, используйте защиту препроцессора. Если другой мод объявляет символ в своём `config.cpp` (через массив `defines[]` в `CfgMods`), вы можете проверить его на этапе компиляции.

### Как это работает

Мод явно объявляет свои символы препроцессора через массив `defines[]` внутри своей записи `CfgMods`. Например, если мод имеет:
...
Тогда `#ifdef MYMOD_AI` будет `true`, когда этот мод загружен.

Обратите внимание, что имена классов `CfgPatches` регистрируют контент аддона мода, но **не** создают символы `#ifdef` --- только `defines[]` это делает. Имена символов выбираются автором мода и не обязаны совпадать с каким-либо именем класса, поэтому соглашения различаются --- проверяйте документацию мода или `config.cpp`.
