...
class CfgSoundSets
{
    class MyMod_GunShot_SoundSet
    {
        ...
        volumeCurve = "InverseSquare2Curve"; // Имя класса кривой затухания из CfgSoundCurves
...
| `volumeCurve` | string | Имя класса кривой затухания, определённого в `CfgSoundCurves` (например, `"InverseSquare2Curve"`, `"LinearCurve"`, `"defaultAmpAttenuationCurve"`). |
...
| `distanceFilter` | string | Имя класса фильтра затухания по расстоянию/частоте, применяемого на дистанции (например, `"defaultDistanceFreqAttenuationFilter"`), приглушающего далёкие звуки. |
...
### Классы кривых громкости

SoundSets ссылаются на классы кривых затухания через свойство `volumeCurve`. Имена кривых -- это имена классов, определённых в `class CfgSoundCurves` (ванильные звуки DZ определяют их множество), и моды могут либо ссылаться на существующий класс, либо определять собственный. Не существует «голых» предустановок с буквальными именами `"InverseSquare"`, `"Linear"` или `"Logarithmic"`. Распространённые ванильные классы кривых:

| Класс кривой | Поведение |
|------------|----------|
| `"InverseSquare2Curve"` | Реалистичное затухание (громкость падает примерно с квадратом расстояния). Естественное звучание. |
| `"LinearCurve"` | Равномерное затухание от максимума до нуля по дальности. |
| `"defaultAmpAttenuationCurve"` | Громко вблизи, быстро падает на средней дистанции, затем медленно затухает. |
...
Для оружия SoundSet'ы выстрела указываются в массиве `soundSetShot[]` внутри класса режима стрельбы оружия. Используйте `soundSetShotExt[]` для вариантов с глушителем:
...
| UI-звуки через 2D звуковую сцену | VPP Admin Tools | Воспроизводит UI-звуки с помощью `SoundParams` + `SoundObjectBuilder` + `GetGame().GetSoundScene().Play2D()` (см. `VPPNotificationUI.c`) |
