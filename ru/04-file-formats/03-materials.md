...
class Stage2                               // Этап текстуры: детальная карта (базовый цвет _co берётся из текстуры модели / hiddenSelectionsTextures[], а не из Stage)
{
...
| **CalmWater** | Водные поверхности с отражением и преломлением | Специальные водные текстуры |
| **TerrainX** | Поверхности рельефа (пиксельный шейдер; вершинный шейдер -- `Terrain`) | Спутник, маска, слои материалов |
| **NormalMap** | Упрощённая поверхность с картой нормалей | Нормали, Диффузный |
| **NormalMapSpecularMap** | С картой нормалей и спекулярным | Нормали, Диффузный, Спекулярный |
| **SuperHair** | Рендеринг волос персонажа | Диффузный с альфа, специальная просвечиваемость |
| **Skin** | Кожа персонажа с подповерхностным рассеянием | Диффузный, Нормали, Спекулярный |
| **AlphaShadow / AlphaNoShadow** | Прозрачность по альфа-ключу (листва, заборы) с отбрасыванием тени или без него | Диффузный с альфа |

### Шейдер Super (самый распространённый)

Шейдер **Super** -- это стандартный шейдер физически корректного рендеринга, используемый для подавляющего большинства предметов в DayZ. Он ожидает следующие основные этапы текстур (соответствует реальным ванильным rvmat, например `DZ\weapons\ammunition\data\00buck_box.rvmat`):
...
Базовый цвет (`_co`) не назначается через Stage в шейдере Super -- он берётся из базовой текстуры модели или `hiddenSelectionsTextures[]`.
...
| **Stage2** | Детальная карта | `_dt` | Тонкая детализация поверхности (базовый цвет `_co` предоставляется базовой текстурой модели / `hiddenSelectionsTextures[]`, а не Stage) |
...
`renderFlags[]` -- это свойство верхнего уровня RVMAT (наравне с `ambient[]` и `PixelShaderID`). Ванильные значения записываются в PascalCase, например `"NoZWrite"`, `"NoAlphaWrite"`, `"NoColorWrite"` и `"AddBlend"`. Флага `"twoSided"` не существует -- двусторонний рендеринг управляется в другом месте (например, в свойствах граней Object Builder), а не через `renderFlags[]`.
...
Ванильные предметы не используют набор именованных универсальных оверлеев для каждого уровня. Вместо этого каждый уровень здоровья указывает на специфичный для предмета RVMAT (обычно `<item>.rvmat`, `<item>_damage.rvmat` и `<item>_destruct.rvmat`), переиспользуя один и тот же файл на соседних уровнях. Единственный универсальный материал в `DZ\data\data\` -- это `default_destruct.rvmat` (рядом с `default.rvmat`):
...
**Исправление:** Убедитесь, что Stage1 = нормали, Stage2 = детализация, Stage3 = макро, Stage5 = спекулярный (для шейдера Super). Базовый цвет `_co` берётся из текстуры модели / `hiddenSelectionsTextures[]`, а не из Stage. См. таблицу назначений этапов выше.
...
**Исправление:** Используйте шейдер `Glass`, `AlphaShadow` или `AlphaNoShadow` вместо `Super` для прозрачных поверхностей. Используйте текстуры с суффиксом `_ca` с правильными альфа-каналами.
...
- **Версионность:** Текстовый формат RVMAT и имена основных шейдеров (Super, Glass, Multi) стабильны с DayZ 1.0. Однако последние обновления всё же добавили структуру: проекционный слой Sakhal ввёл новые ключи верхнего уровня (`superAllowProjectionLayer`, `degAngleTopProjectionStart`/`End`), классы `TexGen` и дополнительные этапы (например, `Stage10`) для шейдеров Super, TreeAdvTrunk и Multi.
