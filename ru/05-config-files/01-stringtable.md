...
VPP использует сокращённый набор столбцов (14 столбцов, без столбца `hungarian`) и не добавляет к ключам префикс `STR_`:
...
Это демонстрирует, что префикс `STR_` является соглашением, а не требованием. Разрешение с префиксом `#` в файлах макетов работает с любым ключом из stringtable --- оно не зависит от префикса `STR_`. VPP ссылается на эти ключи через атрибут `loc` в своём inputs.xml (например, `loc="vpp_focus_on_game"`), а также через код скрипта. Префикс `STR_` всё же настоятельно рекомендуется для всех новых модов.
