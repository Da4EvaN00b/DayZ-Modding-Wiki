...
| `simpleChildrenUseDefaultAttributes` | bool | Если `true`, простые дочерние элементы используют значения по умолчанию конфигурации. Если `false`, они используют `attributes` родителя |
...
| `attributes` | object | Диапазоны здоровья/количества по умолчанию. Используются дочерними элементами, когда `simpleChildrenUseDefaultAttributes` равно `false` |
...
| `simpleChildrenUseDefaultAttributes` | bool | Если `true`, простые дочерние элементы используют значения по умолчанию конфигурации. Если `false`, они используют `attributes` этой структуры |
...
- **`true`** --- Предметы используют значения по умолчанию конфигурации движка (обычно полное здоровье и количество).
- **`false`** --- Предметы используют `attributes`, определённые в родительской структуре.
...
| Использование `spawnGearPresetFiles` без удаления кода `StartingEquipSetup()` | Скриптовый набор молча переопределяется JSON-пресетом. Когда активны валидные пресеты, `StartingEquipSetup()` вообще не вызывается --- его предметы не создаются и затем не заменяются | Это ожидаемое поведение, не баг. Удалите или закомментируйте код загрузки в init.c во избежание путаницы |
