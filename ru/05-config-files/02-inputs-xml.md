...
Вы можете разместить `inputs.xml` в любом месте внутри PBO вашего мода. Распространённая структура --- подпапка `data` вашей директории Scripts:
...
Расположение файла не фиксировано соглашением; движок не обнаруживает его автоматически. Вы должны зарегистрировать файл, указав его в свойстве `inputs` блока `CfgMods` вашего `config.cpp`, например `inputs = "MyMod/Scripts/data/inputs.xml";`. Путь произвольный --- движок загружает файл из того места, которое вы укажете.
...
Параметр `false` в `LocalPress("name", false)` --- это аргумент `check_focus`. Передача `false` обрабатывает ввод даже когда окно игры не в фокусе; когда он равен `true` (значение по умолчанию), игра не в фокусе возвращает `false`. Он не управляет поглощением ввода.
...
| Модификаторы | `kLControl`, `kRControl`, `kLShift`, `kRShift`, `kLMenu` (левый Alt), `kRMenu` (правый Alt) |
| Навигация | `kUp`, `kDown`, `kLeft`, `kRight`, `kHome`, `kEnd`, `kPrior` (Page Up), `kNext` (Page Down) |
| Редактирование | `kReturn`, `kBackspace`, `kDelete`, `kInsert`, `kSpace`, `kTab`, `kEscape` |
| Цифровая клавиатура | `kNumpad0` ... `kNumpad9`, `kNumpadEnter`, `kAdd` (numpad +), `kSubstract` (numpad -, обратите внимание на написание в движке), `kMultiply` (numpad *), `kDivide` (numpad /), `kDecimal` (numpad .) |
| Пунктуация | `kMinus`, `kEquals`, `kLBracket`, `kRBracket`, `kBackslash`, `kSemicolon`, `kApostrophe`, `kComma`, `kPeriod`, `kSlash`, `kGrave` |
| Блокировки | `kCapital` (Caps Lock), `kNumlock` (обратите внимание на строчную `l`), `kScrollLock` |
...
| `mB4` | Кнопка мыши 4 (боковая кнопка назад) |
| `mB5` | Кнопка мыши 5 (боковая кнопка вперёд) |
| `mB6`, `mB7`, `mB8` | Дополнительные кнопки мыши |

### Перемещение мыши и колесо

| Имя | Направление |
|------|-----------|
| `mLeft` | Мышь перемещена влево |
| `mRight` | Мышь перемещена вправо |
| `mUp` | Мышь перемещена вверх |
| `mDown` | Мышь перемещена вниз |
...
- **Перемещение/колесо мыши**: префикс `m` + имя направления (например, `mLeft`, `mWheelUp`)
...
| Комбинации с модификаторами через вложенные `<btn>` | Внешний --- модификатор, внутренний --- триггер | Клавиша-модификатор сама по себе также регистрируется как нажатие на свою собственную привязку (например, `kLControl` --- это стандартная задержка дыхания, привязанная к `UAHoldBreath`; стандартное приседание/смена стойки `UAStance` находится на `kC`). Игрок, удерживающий Ctrl+Клик, также активирует задержку дыхания |
