...
| Симптом | Причина | Исправление |
|---------|---------|-------------|
| Макет загружается, но ничего не видно | Размер виджета равен нулю | Проверьте атрибут `size` виджета (значения `w h` должны быть больше нуля). Не используйте отрицательные размеры. (`hexactsize`/`vexactsize` — это флаги `0`/`1`, которые выбирают пропорциональный (`0`) или пиксельный (`1`) режим размера, а не сам размер.) См. [Главу 3.3](03-gui-system/03-sizing-positioning.md). |
...
| Текст показывает `#STR_some_key` буквально | Запись в stringtable отсутствует или файл не загружен | Добавьте ключ в ваш `stringtable.csv`. Проверьте, что CSV находится в корне мода и имеет правильный формат заголовка `"Language","original","english",...` (первый столбец содержит ключ строки, `original` — это запасной вариант, а остальные столбцы — переводы по языкам). См. [Главу 5.1](05-config-files/01-stringtable.md). |
...
| Изменения `cfgGameplay.json` игнорируются | Файл не включён или находится в неверном месте | Поместите файл в папку миссии. Установите `enableCfgGameplayFile` в `1` в `serverDZ.cfg`. Перезапустите сервер (не просто перезагрузку). |
...
| Предметы исчезают из хранилища | `lifetime` истёк в `types.xml` | Увеличьте `lifetime` для персистентных предметов. Значение по умолчанию часто слишком короткое для контейнеров строительства баз. Проверьте значение `CleanupLifetimeRuined` в `globals.xml`. |
