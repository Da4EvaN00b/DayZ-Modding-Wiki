...
影响整个经济系统的全局参数：僵尸数量、动物数量、清理计时器、战利品损坏范围、重生时间。共有 30 个参数。完整参考请见 [Chapter 9.4](04-loot-economy.md)。
...
```
storage_1/
  players/           # 二进制角色记录，每个玩家一个
  spawnpoints.bin    # 二进制出生点数据
  backup/            # 持久化数据的自动备份
  data/              # 世界状态（放置的物品、基地建筑、载具）
```

**服务器运行时不要手动编辑 `players/` 中的文件。** 这些是仅由服务器进程写入的不透明二进制记录。如果需要清除角色数据，请先停止服务器，然后删除或重命名该文件夹。

要进行**完全清档**，停止服务器并删除整个 `storage_1/` 文件夹。服务器在下次启动时会以全新世界重建它。

要进行**部分清档**（保留角色，重置战利品）：
1. 停止服务器
2. 删除 `storage_1/data/` 中的文件但保留 `storage_1/players/`
3. 重启
