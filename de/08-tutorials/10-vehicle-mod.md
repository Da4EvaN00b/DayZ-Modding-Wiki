...
**OnDebugSpawn** -- Aufgerufen wenn you use the debug spawn menu. `SpawnUniversalParts()` adds headlight bulbs and a car battery. `FillUpCarFluids()` fills fuel, coolant, and oil to maximum (it does not fill brake fluid). We then create wheels, doors, hood, and trunk. This gives you an immediately drivable vehicle for testing.
