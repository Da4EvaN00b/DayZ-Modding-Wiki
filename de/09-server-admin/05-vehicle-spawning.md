...
`events.xml` befindet sich in `mpmissions/<Ihre_Mission>/db/`, waehrend `cfgeventspawns.xml` und `cfgeventgroups.xml` im Missions-Stammverzeichnis liegen (`mpmissions/<Ihre_Mission>/`).
...
Konvois funktionieren aehnlich wie Heliabstuerze: Das `<secondary>`-Tag spawnt `InfectedArmy` um die Stelle, und Loot-Items mit `deloot="1"` erscheinen an den Wracks. Anders als beim Heliabsturz hat das Konvoi-Event ein leeres `<children/>`-Element -- seine zerstoerten Fahrzeuge sind als Gruppe in `cfgeventgroups.xml` definiert und werden ueber Gruppenreferenzen in `cfgeventspawns.xml` platziert. Mit `nominal=5` existieren bis zu 5 Konvoistellen gleichzeitig auf der Karte. Jeder bleibt 1800 Sekunden (30 Minuten), bevor er an einen neuen Ort wechselt.
...
Jede Gruppe wird mit einem `<group name="...">`-Element innerhalb des Datei-Stammelements `<eventgroupdef>` deklariert, und ihre `<child>`-Eintraege sind direkte Kinder von `<group>` (hier gibt es keinen `<children>`-Wrapper). Das erste Kind wird an der Position aus `cfgeventspawns.xml` platziert. Nachfolgende Kinder werden um ihre `x`-, `z`-, `y`-, `a`-Werte relativ zu diesem Ursprung versetzt.
...
| `y` | Y-Offset (vertikal) in Metern vom Gruppenursprung. |
| `a` | Winkeloffset in Grad vom Gruppenursprung. |
| `deloot` | Ob dynamischer Event-Loot in diesem Kind spawnen kann (0 oder 1). |
| `lootmin` | Mindestanzahl von Loot-Items, die in diesem Kind gespawnt werden. |
| `lootmax` | Maximale Anzahl von Loot-Items, die in diesem Kind gespawnt werden. |
