...
Die zentralen CE-Dateien (`types.xml`, `events.xml`, `globals.xml`) liegen per fest eingebauter Konvention im Ordner `db/` --- die vanilla `cfgeconomycore.xml` enthält kein `<ce>`-Element, das auf sie verweist. Das `<ce>`-Element wird stattdessen verwendet, um **zusätzliche** benutzerdefinierte CE-Dateien zu registrieren (eingeführt in Update 1.08): Das Attribut `folder` benennt den Ordner, der dein benutzerdefiniertes XML enthält, und jeder verschachtelte `<file name="..." type="..."/>`-Eintrag ergänzt die passende vanilla Datei oder überschreibt sie (`type` kann `types`, `spawnabletypes`, `globals`, `economy`, `events` oder `messages` sein).
