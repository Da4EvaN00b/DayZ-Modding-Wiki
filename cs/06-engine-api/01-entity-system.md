...
```c
bool ConfigGetBool(string entryName);   // běžná skriptová metoda: vrací ConfigGetInt(entryName) == 1
...
```c
ComponentEnergyManager GetCompEM();   // běžná skriptová metoda (vrací m_EM nebo přetypuje komponentu energy manageru), není proto native
```
